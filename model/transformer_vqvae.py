...
from model.quantize import VectorQuantizer
...
class Stack(nn.Module):
    ...
    def __init__(self, block_size, n_layer=12, n_head=8, n_embd=256,
        ...
        for i in range(config.n_layer):
            ...
            self.blocks.append(STBlock(config,
...
class TransformerAutoEncoder(nn.Module):
    ...
    def __init__(self, *, in_dim=1024, n_layers=6,
        ...
        self.quant_emb = nn.Linear(n_embd*num_joints, e_dim)
        self.encoder_dim = e_dim
        self.post_quant_emb = nn.Linear(e_dim, n_embd*num_joints)
...
class TransformerVQVAE(TransformerAutoEncoder):
    ...
    def forward(self, *, x, y, valid, quant_prop=1.0, **kwargs):
        ...
        z = self.quant_emb(hid.view(batch_size,seq_len, -1))
        ...
        hid= hid.view(batch_size, seq_len, self.num_joints,-1)
...
if __name__=="__main__":
    ...
    input=torch.randn(1,64,32,6).to(device)
    output=model(x=input,y=0,valid=torch.ones(1,64).to(device))
    ...
    # 计算 FLOPs 和参数量
    macs, params = profile(wrapped_model, inputs=(input, 0, torch.ones(1,64).to(device)))
